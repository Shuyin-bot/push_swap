...
/*   Updated: 2026/05/04 08:36:04 by qianshuyin       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <push_swap.h>

static t_stack	*init_turk_sort(t_stack *a);
static void		turk_sort_rotate_b(t_stack *a, t_stack *b, t_node *node);
static void		finish_turk_sort(t_stack *a, t_stack *b);
...
static void	turk_sort_rotate_b(t_stack *a, t_stack *b, t_node *node)
{
	while (node->index != 0)
	{
		...
		{
			if (node->target_in_a->index <= a->size / 2
				&& node->target_in_a->index != 0)
				rr(a, b);
			else
				rb(b);
		}
